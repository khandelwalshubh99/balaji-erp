...
export const title = 'Tally orders';

let windowDays = 45;
let filter = 'pending';

export async function render(el) {
  const d = await api(`/tally-orders?days=${windowDays}`);
  ...
  el.innerHTML = html`
    <div class="callout" style="margin-bottom:14px">
      <strong>This is inference, not a shared record.</strong> It reads sales orders straight out of Tally and calls
      one dispatched only because a delivery note quotes its sales-order number — type the reference differently and
      it looks pending forever. It stays useful for orders that were placed before this system, and for anything
      raised directly in Tally. Orders taken here live under <strong>Orders</strong>, where nothing is inferred.
    </div>
